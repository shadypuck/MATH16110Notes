...
\begin{enumerate}[ref={\thechapter.\arabic*}]
    \item \label{axr:0.1}\marginnote{10/13:}Prove that if $A$ is a non-empty subset of $\N$, then $A$ has a least element, i.e., there is some $n_0\in A$ such that for all $n\in A$, we have $n_0\leq n$.
    ...
    \end{enumerate}
    \setcounter{enumi}{6}
    \item \label{axr:0.7}\marginnote{10/8:}Let $n$ be a natural number and $k\leq n$ also be a natural number. Define
    \begin{equation*}
    ...
    \end{proof}
    \item \label{axr:0.8}\marginnote{10/15:}Let $a,b\in\Z$. We say that $b$ is \textbf{divisible} by $a$ if there is an integer $m$ such that $b=ma$. It is easily shown that
    ...
    \end{enumerate}
    \item \label{axr:0.9}\marginnote{10/8:}From Peano's Postulates (below), prove the following claims.
    ...
    \begin{enumerate}[label={(\alph*)},ref={\theenumi\alph*}]
        ...
        \begin{proof}[Proof of ii]
            ...
            \end{align*}
        \end{proof}
        \item \label{axr:0.9f}\marginnote{10/6:}\leavevmode\vspace{-1.6em}
        \begin{definition*}
            For $n\in\N$ and $k\in\N_0$, we define $n^k$ inductively by
            \begin{align*}
                n^0 &= 1\\
                n^{k+1} &= n\cdot n^k
            \end{align*}
